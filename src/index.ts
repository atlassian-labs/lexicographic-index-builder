...
export type PartitionKeyOptions<Entity, PK extends (keyof Entity)[]> = {
  prefix: string;
  fields: PK;
  transformValues?: TransformValues<Pick<Entity, AnyElementOf<PK>>>;
};
export type SortKeyOptions<Entity, SK extends (keyof Entity)[]> = {
  prefix: string;
  fields: SK;
...
const isEmptySegment = (segment: Segment | undefined | null) => segment === null || segment === undefined;
 ...
 */
export class GSIBuilder<Entity, PK extends (keyof Entity)[] = [], SK extends (keyof Entity)[] = []> {
  constructor(
  ...
  public withEncodingMethod(encodingMethod: EncodingMethod): GSIBuilder<Entity, PK, SK> {
  ...
  public withPartitionKey<PK extends (keyof Entity)[]>(
    ...
    return new GSIBuilder<Entity, PK, SK>({ ...this.state, partitionKeyOptions });
  }
  ...
  build(): GSI<Entity, PK, SK> {
    ...
    return new GSI<Entity, PK, SK>(state.name, state.partitionKeyOptions, state.sortKeyOptions, state.encodingMethod);
  }
}

export class GSI<Entity, PK extends (keyof Entity)[], SK extends (keyof Entity)[]> {
  constructor(
  ...
  public partitionKey(partitionKeyFields: Pick<Entity, AnyElementOf<PK>>) {
    const transformedPk = this.partitionKeyOptions.transformValues?.(partitionKeyFields) || partitionKeyFields;
    const segments = this.partitionKeyOptions.fields.map(field => transformedPk[field] as unknown as Segment);
  ...
  public sortKey(sortKeyFields: Partial<Pick<Entity, AnyElementOf<SK>>> = {}) {
    const transformedSk = this.sortKeyOptions.transformValues?.(sortKeyFields) || sortKeyFields;
